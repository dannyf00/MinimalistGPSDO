...
// - v0.7a,12/07/2024: completed documentation of the code - finally!
//
//
//
//               dsPIC33FJ
...
//              |                     |
//              |                 RB5 |>---------->LED/PWM_OUT (PWM2) --->[100k]---+---[100k]----+---[C1:22pf]---+--->OSC_OUT/[OSC_IN]
//              |                     |                                            |             |               |
...
//  1. the MCU acts as a frequency meter measuring its own VCXO output frequency against a reference PPS signal (from a GPS for example)
//  2. based on the measured frequency error, the MCU adjusts the control voltage to the VCXO to minimize the frequency error
//
//documentation
...
//  1.    set up the project with C2 // C1 + BB910 combined capacitance approximate the crystal's load capacitance.
//  2.    comment out PPSDO_RUN -> this allows the GPSDO to run open loop
//  3.    measure the frequency sensitivity against control
//  3.1   set PWM2 duty cycle to  1/16, and record tksErr reading on the serial monitor (=tksErr1)
//  3.2   set PWM2 duty cycle to 15/16, and record tksErr reading on the serial monitor (=tksErr2)
//  3.3   adjust C1 so that you have the desired range. I typically set that range to cover the crystal's frequency accuracy
//  3.3   re-iterate 3.1 .. 3.3 until you have achieved the desired range.
//  3.4   calculate the sensitivity as (65536 / (15 - 1)) / (tksErr2 - tksErr1).
//  3.5   the sensitivity calculated sets the upper limit for cPID_Px. I usually set cPID_Px to half of the calculated sensitivity
//  3.5   if the sensitivity calculated above is negative, comment out VTRLSLOPE_P.
//  4.    adjust the center frequency
//  4.1   set PWM2 duty cycle to 8/16, and watch tksErr on the serial monitor
//  4.2   adjust C2 until tksErr is minimized
//  5.    run the GPSDO closed loop by removing the comment on PPSDO_RUN.
//  5.1   fine-tune the coarse PID gains (cPID_Px, cPID_Ix and cPID_Dx).
//  5.1.1 set cPID_Px to half of the calculated sensitivity in 3.4
//  5.1.2 adjust cPID_Ix so that the PID_I term on the serial monitor doesn't oscillate.
//  5.1.3 do the same for cPID_Dx term.
//  5.2   once your GPSDO locks onto the reference PPS signal, the fine PID loop kicks in.
//  5.3   the stock numbers are pretty good but you can use the same principle to adjust them to your particular setup.
//
...
#define PPSDO_RUN						//if define, this runs normally. if commented out, goes into calibration mode
...
//misc signatures
#define PPSDO_BRAND	"GPSDO by dannyf"
#define PPSDO_VER	"v0.7a, as of 12/7/2024"
#define PPSDO_CR	"all rights reserved"

#define PPSDO_HEADER					//send header over uart, if defined. if commented out, the header is muted
...
//cPID_Px usually less than 2048 - 4096
//for my system, the measured sensitivity is about 1hz -> 1.3k dc#). It oscillates if cPID_Px = 4096
#define cPPS_OVN	2					//oversample. gate time. practically unlimited
#define cPID_Px		1024				//PID gain for P_term, normalized by PID_x: 64 .. 1/2 Sensitivity (usually less than 2048)
#define cPID_Ix		256					//PID gain for I_term, normalized by PID_x: 0..cPID_Px/2 (usually less than cPID_Px / 16)
#define cPID_Dx		32					//PID gain for D_term, normalized by PID_x: 0..cPID_Ix (usually less than cPID_Px / 256)
#define cPID_x		(cPPS_OVN)			//normalizer for PID_gains: may use it if the gain / timer period is too big (PPS_OVN >= 256)
...
//good practice to anchor fPID parameters to cPID parameters
//fPPS_OVN must fully divide 1000000ul: factors of 2 and 5
#define fPPS_OVN	10					//oversample. gate time. practically unlimited
#define fPID_Px		(cPID_Px / 1)		//PID gain for P_term, normalized by PID_x: /1 .. /16
#define fPID_Ix		(cPID_Ix / 1)		//PID gain for I_term, normalized by PID_x: /1 .. /16
#define fPID_Dx		(cPID_Dx / 2)		//PID gain for D_term, normalized by PID_x: /1 .. /32
#define fPID_x		(fPPS_OVN)			//normalizer for PID_gains: may use it if the gain / timer period is too big (PPS_OVN >= 256)

//#define PPSDO_DEBUG						//use if we are debugging - enabling print out from errPID(). comment out if not used
#define TKS_OFST	(-100+8*1)			//tks offset - for debugging: adjust this value so that the target frequency is within range of the Vtrl adjustments
...
volatile int32_t tksErr_i, tksErr_s;	//frequency error over PPS_OVN seconds, _i: instant, _s: smoothed
...
void ppsInit(void) {
	...
	uart2Wait(); uart2Puts(PPSDO_BRAND); uart2Puts(", "); uart2Puts(PPSDO_VER); uart2Puts(". "); uart2Puts(PPSDO_CR); uart2Puts(".\r\n");
	uart2Wait(); uart2Puts("compiled: "); uart2Puts(__TIME__); uart2Puts(", "); uart2Puts(__DATE__); uart2Puts(".\r\n");
	uart2Putline("key parameters:");
	uart2Wait(); u2Print("Fgpsdo=                  ", F_CPU);	u2Println();
	uart2Wait(); u2Print("PBLCK =                  ", PPB_LOCK); u2Print("PSLCKN=                  ", PPS_LOCKN); u2Println();
	uart2Wait(); u2Print("cOVN  =                  ", cPPS_OVN); u2Print("cPIDPx=                  ", cPID_Px); u2Print("cPIDIx=                  ", cPID_Ix); u2Print("cPIDDx=                  ", cPID_Dx); u2Println();
	uart2Wait(); u2Print("fOVN  =                  ", fPPS_OVN); u2Print("fPIDPx=                  ", fPID_Px); u2Print("fPIDIx=                  ", fPID_Ix); u2Print("fPIDDx=                  ", fPID_Dx); u2Println();
#if defined(PPSDO_DEBUG)
	uart2Puts("PPSDO debug mode on.\r\n");
#else
	uart2Puts("PPSDO debug mode off.\r\n");
#endif

	uart2Puts("UART command module not supported"); u2Println(); uart2Wait();
#if defined(PPS_OUT)
...
#if defined(PPSDO_RUN)
	uart2Puts("PPSDO runs closed loop...\r\n");
#else
	uart2Puts("PPSDO runs open loop...\r\n");
#endif
...
void loop(void) {
	...
	if (ppsFlg) { ppsFlg = 0;						//if new data has arrived
...
#endif	//led_lock
#else	//calibrate the range of Vtrl
		//test the range of Vctrl / pwm2 output, DC
		//C2  //C1   +Varactor
		//15pf//100pf+1n4001: 1= 1  tksErr, 15= 50 tksErr. adjustment range=10ppm, or +/- 5ppm, or 3hz/(1/16th) of duty cycle ->1.2k dc/1hz
		//15pf//100pf+redLED: 1= 98 tksErr, 15=112 tksErr. adjustment range= 1ppm, or +/-.5ppm, or.3hz/(1/16th) of duty cycle ->4.1k dc/1hz
		//15pf//100pf+bb910:  1=-32 tksErr, 15= 22 tksErr. adjustment range=11ppm, or +/- 6ppm, or 3hz/(1/16th) of duty cycle ->1.1k dc/1hz
		// 0pf//100pf+bb910:  1=108 tksErr, 15=176 tksErr. adjustment range=14ppm, or +/- 7ppm, or 5hz/(1/16th) of duty cycle ->0.8k dc/1hz
		// 0pf// 15pf+bb910:  1=386 tksErr, 15=389 tksErr. adjustment range=.6ppm, or +/-.3ppm, or.2hz/(1/16th) of duty cycle ->19.k dc/1hz
		//15pf// 15pf+bb910:  1= 38 tksErr, 15= 51 tksErr. adjustment range= 3ppm, or +/- 2ppm, or.9hz/(1/16th) of duty cycle ->4.4k dc/1hz
		//15pf// 47pf+bb910:  1=-46 tksErr, 15=-24 tksErr. adjustment range= 4ppm, or +/- 2ppm, or 2hz/(1/16th) of duty cycle ->2.6k dc/1hz
		// 0pf// 47pf+bb910:  1=-46 tksErr, 15= 11 tksErr. adjustment range= 4ppm, or +/- 2ppm, or 2hz/(1/16th) of duty cycle ->1.0k dc/1hz
		// 0pf// 15pf+bb910:  1= 62 tksErr, 15= 69 tksErr. adjustment range= 1ppm, or +/- 1ppm, or 1hz/(1/16th) of duty cycle ->8.2k dc/1hz
		//batch 2 of bb910 (fake):
		//15pf// 47pf+bb910:  1=-26 tksErr, 15= 15 tksErr. adjustment range= 8ppm, or +/- 4ppm, or 3hz/(1/16th) of duty cycle ->1.4k dc/1hz
		//
		//calibration procedure:
		//1. comment out PPSDO_RUN: this allows an open loop to measure Vtrl sensitivity;
		//1.1. set Vtrl PWM duty cycle to  1/16th; run the code and see tksErr reading 1.
		//1.2. set Vtrl PWM duty cycle to 15/16th; run the code and see tksErr reading 2.
		//1.3. Vtrl sensitivity = 14 * 4096 / (tksErr reading 2 - tksErr reading 1), or DC count per 1Hz frequency error.
		//1.4. Set cPID_Px to a number less than Vtrl sensitivity calculated above. I usually use 1/2 or 1/4.
		//2. measure TKS_OFST: for debugging only; setting this correct allows to fine tune the PID loop;
		//2.1 set Vtrl PWM duty cycle to 50% (8/16th); run the code and see tksErr reading 3.
		//2.2 set TKS_OFST to tksErr reading 3.
		//3. tune PID:
		//3.1 uncomment PPSDO_RUN: this allows the control loop to close;
		//3.2 start with cPID_Ix at approximately 1/16th of cPID_Px, and cPID_Dx to be 1/64th of cPID_Px. Run the code.
		//3.3 should converge quickly to tksErr = 0.
		//3.3.1 you can turn on PPSDO_DEBUG and see the Px_term, Ix_term and Dx_term output on the serial terminal.
		//3.3.2 if they take too long to converge, increase the corresponding cPID_Xx gain; if they oscillate too much, decrease the gains.
		//3.4 once the cPID_Xx gains have been set, fine tune fPID_Xx gains.
		//3.4.1 the starting point is to set them the same, or slightly smaller: 1/2x or 1/4x should work for most cases.
		pwm2SetDC(pwm2GetPR()*1/16);
#endif
		...
		u2Print("tksErr=                  ", tksErr);							//display tksErr
		//uart2Puts("Fgpsdo ="); uart2Puts(freq.str); uart2Puts("   "); 			//display the frequency in string
		//u2Print("FerrmH=                  ", (freq.Hz - F_CPU) * 1000 + (freq.uHz/1000)); 							//display cpu frequency
